...
export default function SettlementHistory() {
  ...
  const getVal = (vals: SettlementValue[], fieldId: string): number => {
    const v = vals.find((x) => x.field_id === fieldId)?.value
    return parseFloat(v || '') || 0
  }

  // Cash multiplier calculation matching Settlement.tsx
  const computeSession = (vals: SettlementValue[]) => {
    ...
    return { posTotal, orderCount, expectedTotal, actualTotal, diff, cashTotal, safeTotal, paymentBreakdown, avgPrice }
  }
