...
export default function StoreSchedules() {
  ...
  useEffect(() => {
    if (!supabase || !session?.staffId || !scope) return
    supabase
      .from('user_pins')
  ...
  }, [canSchedule, staffIds, approverOrder, session?.staffId])
      ...
      {canSchedule && approverOrder !== null && pendingForManager.length > 0 && (
        <div className="px-4 py-3 space-y-2 no-print">
          ...
          {pendingForManager.map((req) => {
            const name = staffList.find((s) => s.id === req.staff_id)?.name || req.staff_id
            return (
              <LeaveRequestCard
                key={req.id}
                request={req}
